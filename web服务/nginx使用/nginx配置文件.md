...
│   ├── mime.types
│   ├── mime.types.default
│   ├── nginx.conf           # 相当于apache的httpd.conf ，默认主配置文件。静态，
...
│   └── win-utf
├── fastcgi_temp
├── html                     # 默认站点目录，类似apache的htdoc
│   ├── 50x.html             # 50x页面
│   └── index.html           # 默认的首页文件，在实际环境中，大家习惯（注意字眼不是必须的）index.html、index.php
                             # index.jsp来做网站的首页文件，首页文件是在nginx.conf中事先定义好的。具体参数为 ind
                             # index index.html index.htm 注意与apache的参数不同，Directory index.html index.htm
├── logs                     # 日志文件夹
│   ├── access.log           # 访问日志
│   ├── error.log            # 错误日志
...
```
#定义Nginx运行的用户和用户组
user nginx nginx;  # 默认用户数nobody apache默认用户是daemon

# nginx进程数，建议设置为等于CPU总核心数。 #看cpu个数cat /proc/cpuinfo |grep processor|wc -l
worker_processes 4;
